...
LetiFramework.Ui.advice.prototype = {
	...
	create: function() {		
        ...
        for (var i = 0; i < this.adviceText.length; i++) {
            ...
            var adviceText = this.game.add.text(this.game.world.centerX, 100 + panel1.y + i * 50, token, style);
            adviceText.anchor.set(0.5);
            var delay = 500;
            var tween = this.game.add.tween(adviceText).from({alpha: 0, y: panel1.y + panel1Height}, delay, Phaser.Easing.Linear.None, true, delay * i);
        }
