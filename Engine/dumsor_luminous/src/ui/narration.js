...
LetiFramework.Ui.narration = function(game) {}

LetiFramework.Ui.narration.prototype = {
	init: function() {
		LetiFramework.Analytics.trackPage("Narration");

		this.screenConfig = LetiFramework.GameController.screensConfig.narration;
		...
		this.narrationSidePanelImage = this.currentGamePageData.narration_side_panel_image;
		this.pageComponents = [];
		this.soundObjects = [];
		this.pageSprites = {};
	...
	preload: function() {
		...
		this.game.load.image(this.screenConfig.narration_side_panel.read_button.image, 
			this.getScreenConfigImagePath(this.screenConfig.narration_side_panel.read_button.image)); 
		this.game.load.image(this.screenConfig.narration_side_panel.narr_next_button.image, 
			this.getScreenConfigImagePath(this.screenConfig.narration_side_panel.narr_next_button.image)); 
		if(this.narrationSidePanelImage && this.narrationSidePanelImage.length > 0) {
			this.loadImageFile(this.narrationSidePanelImage);
		} else {
			this.game.load.image("narrationTextPanel", this.getScreenConfigImagePath(this.screenConfig.narration_side_panel.image));
		}
			...
			if(!alreadyEarnedBadge && awardBadge) {
                ...
                if(rs.length > 0) {
                    ...
                    var model = new LetiFramework.DbEntities.Badge(this.currentUser.id, badge.id, 
                    	this.currentGame.storyId, this.currentGame.id, badge.image, badge.text, 
                    	badge.action, badge.message);
	...
	create: function() {
        ...
        this.bg.events.onInputDown.add(function(){ 
        	if(this.screenConfig.narration_side_panel && this.narrationTextShown) {
        		this.narrationTextShown = false;
        		this.game.add.tween(this.scene).to({x: 0}, 1000, Phaser.Easing.Exponential.Out, true); 
        	}
        }, this);
		...
		if(this.narrations.length > 0) {
			var narrW = this.screenConfig.narration_side_panel.width;
			var halfNarrW = narrW * 0.5;
			var textWidth = narrW - 20;

			var readBtPosition = this.screenConfig.narration_side_panel.read_button.position;
			this.readBt = this.game.add.button(readBtPosition.x, readBtPosition.y, this.screenConfig.narration_side_panel.read_button.image, function() {
				LetiFramework.Analytics.trackEvent("Narration", "Button Click", "Read", 0);
				this.narrationTextShown = !this.narrationTextShown;
    			this.game.add.tween(this.scene).to({x: this.narrationTextShown ? narrW : 0}, 1000, Phaser.Easing.Exponential.Out, true);     	
	        }, this, 2, 1, 0);
	    	this.readBt.input.useHandCursor = true;
	    	this.scene.add(this.readBt);

	        var narrationTextPanel = this.game.add.sprite(-narrW, 0, 
	        	this.narrationSidePanelImage && this.narrationSidePanelImage.length > 0 ? 
	        	this.getAssetPath(this.narrationSidePanelImage) : "narrationTextPanel");
	        narrationTextPanel.scale.setTo(narrW / narrationTextPanel.width, this.game.height / narrationTextPanel.height);
	        var style = this.currentGamePageData.text_style;
	        style.wordWrap = true;
			style.wordWrapWidth = textWidth;
	        this.narrationText = this.game.add.text(-halfNarrW, this.game.world.centerY, this.narrations[this.narrationIdx],  style);
    		this.narrationText.anchor.set(0.5);
	    	this.scene.add(narrationTextPanel);
	    	this.scene.add(this.narrationText);

	    	if(this.narrations.length > 1) {
				var narrNextBtPosition = this.screenConfig.narration_side_panel.narr_next_button.position;
				this.narrNextBt = this.game.add.button(-halfNarrW, narrNextBtPosition.y, this.screenConfig.narration_side_panel.narr_next_button.image, function() {
					LetiFramework.Analytics.trackEvent("Narration", "Button Click", "Next", 0);
					if(++this.narrationIdx < this.narrations.length) {
						this.narrationText.text = this.narrations[this.narrationIdx];
						this.narrationTextShown = true;
						this.scene.x = this.screenConfig.narration_side_panel.width;

						if(this.narrationIdx + 1 == this.narrations.length) {
							this.narrNextBt.destroy();
						}
					}
				}, this, 2, 1, 0);
				this.narrNextBt.x += -0.5 * this.narrNextBt.width;
		    	this.narrNextBt.input.useHandCursor = true;
		    	this.scene.add(this.narrNextBt);
			}
