...
LetiFramework.Ui.decision = function(game) {}

LetiFramework.Ui.decision.prototype = {
	init: function() {
		LetiFramework.Analytics.trackPage("Decision");
		this.screenConfig = LetiFramework.GameController.screensConfig.decision;
        ...
        this.decisions = this.currentGamePageData.decisions;
        ...
        if(this.currentGamePageData.points) {
            ...
            if(rs.length == 0) {  
                ...
                LetiFramework.Analytics.trackEvent("Decision", "Points Score", log, 0);                
	...
	preload: function() {        
        ...
        for(var i = 0; i < this.decisions.length; i++) {
            var decision = this.decisions[i];

            if(decision.badge){
                var rs = jsonsql.query("select * from LetiFramework.GameController.currentStoryBadges where (id==" + 
                    decision.badge.id + ")", LetiFramework.GameController.currentStoryBadges);
                if(rs.length > 0) {
                    var badge = rs[0];
            ...
            this.loadImageFile(decision.pic);

            if(decision.text_background.type == "image") {
                this.loadImageFile(decision.text_background.image);
            }
        }
	...
	create: function() {		
        ...
        this.homeBt = this.game.add.button(optionsBtPosition.x, optionsBtPosition.y, optionsDropDown.home_button.image, 
            function() {
                LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Home", 0);
        ...
        this.soundBt = this.game.add.button(optionsBtPosition.x, optionsBtPosition.y, 
            ...
            function() {
                ...
                if(LetiFramework.SoundManager.narrationOn) {
                    LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Sound On", 0);
                    this.soundBt.loadTexture(this.screenConfig.options_button.children.sound_button.on_image);
                } else {
                    LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Sound Off", 0);
        ...
        this.musicBt = this.game.add.button(optionsBtPosition.x, optionsBtPosition.y, 
            ...
            function() {
                ...
                if(LetiFramework.SoundManager.soundOn) {
                    LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Music On", 0);
                    ...
                    LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Music Off", 0);
        ...
        this.optionsBt = this.game.add.button(optionsBtPosition.x, optionsBtPosition.y, optionsBt.image, 
            function() {
                LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Options", 0);
        ...
        this.prevBt = this.game.add.button(prevBtPosition.x, prevBtPosition.y, this.screenConfig.prev_button.image, 
            function() {
                LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Previous Page", 0);
        ...
        for(var i = 0; i < this.decisions.length; i++) {
            ...
            var decisionSprite = this.game.add.sprite(decision.position.x, decision.position.y, this.getAssetPath(decision.pic));
            decisionSprite.scale.setTo(decision.width / decisionSprite.width, decision.height / decisionSprite.height);

            if(decision.text.length > 0) {
                var decisionTextBg = this.getImageOrShapeBg(decision.text_background);
                decisionTextBg.alpha = decision.text_background.alpha;

                var style = decision.text_style;
                style.wordWrap = true;
                style.wordWrapWidth = decisionTextBg.width - decision.text_background.padding;
                this.decisionText = this.game.add.text(
                    decisionTextBg.x + decisionTextBg.width / 2, decisionTextBg.y + decisionTextBg.height / 2, 
                    decision.text, style);
                this.decisionText.anchor.set(0.5); 
            }

            decisionSprite.inputEnabled = true;  
            decisionSprite.input.useHandCursor = true;
            decisionSprite.events.onInputDown.add(function(src) { 
    ...
    transitionPage: function() {
        var selection = this.decisions[this.selectedChoice].decision;
        LetiFramework.Analytics.trackEvent("Decision", "Button Click", "Next Page", 0);
        var log = {"user": this.currentUser.nickname, "story": this.currentGame.name, 
                    "page": this.currentGamePage,  "decision": selection};
        LetiFramework.Analytics.trackEvent("Decision", "Decision", log, 0);
        this.saveUserGamePlay([selection]);
        if(this.decisions[this.selectedChoice].next_page) {
            LetiFramework.GameController.nextGamePage = this.decisions[this.selectedChoice].next_page;
        }

        var points = this.decisions[this.selectedChoice].points;
        if(points) {
            var rs = LetiFramework.Db.readByKeysAndValues("scores", ["user_id", "game_id", "activity"],
            ...
            if(rs.length == 0) {  
                ...
                var log = {"user": this.currentUser.nickname, "story": this.currentGame.name, 
                                "page": this.currentGamePage, "decision": selection, "points": points};
                LetiFramework.Analytics.trackEvent("Decision", "Points Score", log, 0);                
        ...
        var decisionBadge = this.decisions[this.selectedChoice].badge;
        if(decisionBadge) {
            var count = decisionBadge.count;
            var showBadge = false;
            ...
            if(count == 1) {
                ...
                if(rs.length >= count) {
                    showBadge = true;
                    for (var i = rs.length - 1; i >= rs.length - count; i--) {
                        showBadge &= rs[i].selection[0] == selection;
                    }
                }
            }            
            ...
            if(showBadge) {
                ...
                var alreadyEarned = LetiFramework.Db.readByKeysAndValues("badges", ["user_id", "badge_id"],
                        [this.currentUser.id, decisionBadge.id]).length > 0;
                if(alreadyEarned) {
                    LetiFramework.GameController.nextPage();
                } else {
                    var rs = jsonsql.query("select * from LetiFramework.GameController.currentStoryBadges where (id==" + 
                        decisionBadge.id + ")", LetiFramework.GameController.currentStoryBadges);
                    ...
                    if(rs.length > 0) {
                        ...
                        decisionBadge.text = badge.text;
                        decisionBadge.image = badge.image;
                        decisionBadge.message = badge.message;
                        
                        this.badgeDialog = new LetiFramework.Ui.BadgeDialog(this.game, decisionBadge);
                        this.badgeDialog.show();
                    } else {
                        LetiFramework.GameController.nextPage();
                    }                    
            ...
            if(asset.type == "shape") {             
                ...
                if(asset.start == "auto"){
                    ...
                    cmp.events.onInputDown.add(function(src) {
                        ...
                        LetiFramework.Analytics.trackEvent("Decision", "Interactivity", log, 0);
